"""
Same pipeline as ex1_sim_vs_experiment_savgol.py, but with a wider Savgol
window and lower polyorder so the filter acts as a stronger built-in LPF on
the experiment derivatives. This avoids chaining two filters (which tends to
either be redundant or over-smooth q itself).

Savgol params: window=201 (~400 ms at 500 Hz), polyorder=4 (supports deriv=3).

Output folder: compare_sim_vs_exp_savgol_wide/.
"""
...
SIM_DIR = _find_latest_results("results_adal_vs_quintic_vs_dircol")
# Experimental CSVs (real robot rollouts).
EXP_ADAL_CSV = HW_DIR / "adal_p2p_result.csv"
EXP_QUINTIC_CSV = HW_DIR / "quintic_p2p_result.csv"
# Comparison outputs go next to this script.
OUT_DIR = HERE / "compare_sim_vs_exp_savgol_wide"
OUT_DIR.mkdir(parents=True, exist_ok=True)

T_WINDOW = 2.0            # seconds starting from the motion-onset t=0
SAVGOL_WINDOW = 301       # samples (~500 ms at 500 Hz)
SAVGOL_POLYORDER = 4      # must be > deriv order; with qd-based deriv (max=2), 3 suffices
ONSET_EPS = 1e-9          # ||target_qd|| below this is considered "zero"
MARKER_STEP_S = 0.025      # marker spacing in seconds (markers at 0, 0.025, 0.05, ..., T_WINDOW)
PAD_S = 0.05               # symmetric padding on both sides of displayed [0, T_WINDOW]
END_OFFSET_S = 0.015       # robot settles ~10ms after target_qd reaches zero
...
def load_experiment_csv_savgol(path, t_window=T_WINDOW,
    ...
    t_origin = end_point_abs - t_window                         # displayed t=0 in absolute time
    t_lo = max(t_origin - PAD_S, float(t_abs_u[0]))             # displayed t = -PAD_S
    t_hi = min(end_point_abs + PAD_S, float(t_abs_u[-1]))       # displayed t = T_WINDOW + PAD_S
...
COLOR_QUINTIC = "tab:orange" # tab:orange
...
def plot_joint_trajectories_4way(
    ...
    for j, ax in enumerate(axes):
        ...
        ax.plot(t_qs, q_qs[:, j], "-", color=COLOR_QUINTIC, linewidth=1.2,
                label="Quintic (sim)" if j == 0 else None)
        ax.plot(t_qe, q_qe[:, j], "o", color=COLOR_QUINTIC, markersize=5.0,
                markerfacecolor="none", markeredgewidth=0.8,
                linestyle="None",
                label="Quintic (exp)" if j == 0 else None)
...
def plot_norm_profile_4way(
    ...
    ax.plot(t_qs, n_qs, "-", color=COLOR_QUINTIC, linewidth=1.2, label="Quintic (sim)")
    ax.plot(t_qe, n_qe, "o", color=COLOR_QUINTIC, markersize=5.0,
            markerfacecolor="none", markeredgewidth=0.8,
            linestyle="None", label="Quintic (exp)")
    ax.set_xlabel(r"Time, $t$ (s)")
...
def plot_raw_with_detection(name, t_abs_u, q_raw, qd_raw,
    ...
    so the displayed axis matches the main comparison plots
    (motion end lands at t = T_WINDOW)."""
...
def write_exp_report_docx(docx_path,
                          ...
                          fig_paths, name_a="ADA-L", name_b="Quintic"):
...
def main():
    ...
    sim_quintic_csv = SIM_DIR / "Quintic_trajectory_full.csv"
    for p in (sim_adal_csv, sim_quintic_csv, EXP_ADAL_CSV, EXP_QUINTIC_CSV):
        if not p.exists():
            raise FileNotFoundError(f"Missing input file: {p}")

    t_ls, q_ls, qd_ls, qdd_ls, qj_ls = load_sim_csv(sim_adal_csv)
    t_qs, q_qs, qd_qs, qdd_qs, qj_qs = load_sim_csv(sim_quintic_csv)
    t_le, q_le, qd_le, qdd_le, qj_le = load_experiment_csv_savgol(EXP_ADAL_CSV)
    t_qe, q_qe, qd_qe, qdd_qe, qj_qe = load_experiment_csv_savgol(EXP_QUINTIC_CSV)

    print(f"[sim/ADA-L]     N={len(t_ls)}, t in [{t_ls[0]:.3f}, {t_ls[-1]:.3f}] s")
    print(f"[sim/Quintic] N={len(t_qs)}, t in [{t_qs[0]:.3f}, {t_qs[-1]:.3f}] s")
    print(f"[exp/ADA-L]     N={len(t_le)}, t in [{t_le[0]:.3f}, {t_le[-1]:.3f}] s "
          f"(savgol window={SAVGOL_WINDOW}, poly={SAVGOL_POLYORDER})")
    print(f"[exp/Quintic] N={len(t_qe)}, t in [{t_qe[0]:.3f}, {t_qe[-1]:.3f}] s "
          f"(savgol window={SAVGOL_WINDOW}, poly={SAVGOL_POLYORDER})")
    ...
    metrics_qui_sim = compute_metrics(t_qs, q_qs, qd_qs, qdd_qs, qj_qs)
    metrics_adal_exp = compute_metrics(t_le, q_le, qd_le, qdd_le, qj_le)
    metrics_qui_exp = compute_metrics(t_qe, q_qe, qd_qe, qdd_qe, qj_qe)
    ...
    write_exp_report_docx(docx_path,
                          metrics_adal_sim, metrics_qui_sim,
                          metrics_adal_exp, metrics_qui_exp,
                          fig_entries, name_a="ADA-L", name_b="Quintic")
