...
Extended version:
- terminal constraint = Position + 1 Orientation
- orientation is implemented as tool z-axis alignment:
      z_ee(T) ≈ target_dir
  e.g. downward-facing tool:
      target_dir = [0, 0, -1]

Both are evaluated with the same benchmark metrics plus orientation metrics:
- final_position_error
- final_orientation_error_deg
- final_orientation_vector_error
- joint_path_length
- ee_path_length
...
def save_json(data, path):
    def convert(v):
        ...
        if isinstance(v, (np.float16, np.float32, np.float64)):
            return float(v)
        if isinstance(v, (np.int16, np.int32, np.int64)):
            return int(v)
        return v
...
def safe_unit_tf(v, eps=1e-12):
    v = tf.cast(v, DTYPE)
    n = tf.norm(v)
    return v / tf.maximum(n, tf.constant(eps, dtype=DTYPE))


def orientation_angle_deg(z_axis, target_dir):
    z = safe_unit_np(z_axis)
    d = safe_unit_np(target_dir)
    c = np.clip(np.dot(z, d), -1.0, 1.0)
...
def write_markdown_report(
    ...
    lines.append(f"- target_dir: `{np.array2string(run_config['target_dir'], precision=5)}`")
    ...
    lines.append("## Final tool z-axis directions\n")
    lines.append(f"- ADA-L final tool z-axis: `{np.array2string(adal_final_tool_z, precision=6)}`")
    lines.append(f"- Quintic final tool z-axis: `{np.array2string(quintic_final_tool_z, precision=6)}`")
    lines.append(f"- Target direction: `{np.array2string(target_dir, precision=6)}`\n")
...
def try_write_docx_report(
    ...
    doc.add_paragraph(f"target_dir: {np.array2string(run_config['target_dir'], precision=5)}")
    ...
    doc.add_heading("Final tool z-axis directions", level=1)
    doc.add_paragraph(f"ADA-L final tool z-axis: {np.array2string(adal_final_tool_z, precision=6)}")
    doc.add_paragraph(f"Quintic final tool z-axis: {np.array2string(quintic_final_tool_z, precision=6)}")
    doc.add_paragraph(f"Target direction: {np.array2string(target_dir, precision=6)}")
...
class UR5eKinematics:
    ...
    def forward_all_points(self, q_batch, return_rotation=False):
...
def benchmark_metrics(q, qdot, qddot, qjerk, ee, target_xyz, dt, solve_time,
    ...
    target_dir = safe_unit_np(target_dir)
    final_tool_z_axis = safe_unit_np(final_tool_z_axis)
    ...
    metrics = {
        ...
        "final_orientation_vector_error": float(np.linalg.norm(final_tool_z_axis - target_dir)),
        "final_orientation_error_deg": orientation_angle_deg(final_tool_z_axis, target_dir),
        "final_tool_z_axis": final_tool_z_axis.astype(np.float32),
...
def print_metrics_table(name, metrics):
    ...
    print("final_tool_z_axis               :", np.array2string(metrics["final_tool_z_axis"], precision=5))
...
def print_comparison_table(metrics_a, metrics_b, name_a="ADA-L", name_b="Quintic"):
    ...
    print("\n" + "=" * 108)
    print(f"{'Metric':>32s} | {name_a:>22s} | {name_b:>22s}")
    print("=" * 108)
    for k in keys:
        print(f"{k:>32s} | {metrics_a[k]:22.6e} | {metrics_b[k]:22.6e}")
    print("=" * 108)
...
class ADALTrajectoryPlanner:
    def __init__(
        ...
        target_dir = np.asarray(target_dir, dtype=np.float32).reshape(3,)
        target_dir = target_dir / (np.linalg.norm(target_dir) + 1e-8)
        self.target_dir = tf.constant(target_dir, dtype=DTYPE)
        ...
        self.w_goal = tf.constant(w_goal, dtype=DTYPE)
        self.w_ori = tf.constant(w_ori, dtype=DTYPE)
        ...
        self.w_vel_limit = self.w_limit
        self.w_acc_limit = self.w_limit
    ...
    def terminal_pose_from_q(self, q):
        qT = q[-1:, :]
        _, eeT, rotT = self.robot.forward_all_points(qT, return_rotation=True)
        eeT = eeT[0]
        rotT = rotT[0]
        zT = safe_unit_tf(rotT[:, 2])
        return eeT, rotT, zT
    ...
    def compute_loss_train(self):
        ...
        eeT, rotT, zT = self.terminal_pose_from_q(q)

        goal_loss = tf.reduce_mean(tf.square(eeT - self.target_xyz))
        ori_loss = tf.reduce_mean(tf.square(zT - self.target_dir))
        ...
        lower_violation = tf.nn.relu(q_lo_safe - q)
        upper_violation = tf.nn.relu(q - q_hi_safe)
        limit_loss = tf.reduce_mean(lower_violation + upper_violation)
        ...
        loss = (
            ...
            + self.w_limit * limit_loss
            + self.w_vel_limit * vel_limit_loss
            + self.w_acc_limit * acc_limit_loss
    ...
    def train(self, epochs=1000, lr=3e-2, print_every=200, use_lbfgs=True, lbfgs_maxiter=2000):
        ...
        for ep in range(1, epochs + 1):
            loss, goal, ori, path, acc, jerk, limit, terminal_vel, terminal_acc, initial_acc = self.train_step()

            self.history["loss"].append(float(loss.numpy()))
            self.history["goal"].append(float(goal.numpy()))
            self.history["ori"].append(float(ori.numpy()))
    ...
    def results(self):
        ...
        _, eeT, rotT = self.robot.forward_all_points(q[-1:, :], return_rotation=True)
        tool_z_T = safe_unit_tf(rotT[0, :, 2])
        ...
        return {
            ...
            "target_dir": self.target_dir.numpy(),
            "final_tool_z_axis": tool_z_T.numpy(),
        }
...
class TerminalIKSolver:
    def __init__(self, robot, q0, target_xyz, target_dir=np.array([0.0, 0.0, -1.0], dtype=np.float32),
                 q_min=None, q_max=None, w_goal=20000.0, w_ori=200.0, w_reg=1e-3, w_limit=20.0):
        self.robot = robot
        ...
        self.w_goal = tf.constant(w_goal, dtype=DTYPE)
        self.w_ori = tf.constant(w_ori, dtype=DTYPE)
        self.w_reg = tf.constant(w_reg, dtype=DTYPE)
        self.w_limit = tf.constant(w_limit, dtype=DTYPE)
        ...
        self.history = {
            "loss": [], "goal": [], "ori": [], "reg": [],
            "limit": [], "pos_err": [], "ori_err": [],
        }

    @tf.function
    def step(self):
        with tf.GradientTape() as tape:
            _, ee, rot = self.robot.forward_all_points(self.q_var[None, :], return_rotation=True)
            ee = ee[0]
            rot = rot[0]
            z_axis = safe_unit_tf(rot[:, 2])

            goal_loss = tf.reduce_mean(tf.square(ee - self.target_xyz))
            ori_loss = tf.reduce_mean(tf.square(z_axis - self.target_dir))
            reg_loss = tf.reduce_mean(tf.square(self.q_var - self.q0))
        ...
        ori_err = tf.norm(z_axis - self.target_dir)

        return loss, goal_loss, ori_loss, reg_loss, limit_loss, pos_err, ori_err
    ...
    def solve(self, epochs=3000, lr=3e-2, print_every=300):
        ...
        t0 = time.time()
        for ep in range(1, epochs + 1):
            loss, goal_loss, ori_loss, reg_loss, limit_loss, pos_err, ori_err = self.step()

            self.history["loss"].append(float(loss.numpy()))
            self.history["goal"].append(float(goal_loss.numpy()))
            self.history["ori"].append(float(ori_loss.numpy()))
            ...
            self.history["ori_err"].append(float(ori_err.numpy()))
            ...
            if ep % print_every == 0 or ep == 1:
                print(
                    ...
                    f"ori_err={ori_err.numpy():.6e}"
                )

        elapsed = time.time() - t0
        _, ee, rot = self.robot.forward_all_points(self.q_var[None, :], return_rotation=True)
        ee = ee[0].numpy()
        tool_z = safe_unit_np(rot[0, :, 2].numpy())

        return self.q_var.numpy(), ee, tool_z, elapsed, self.history
...
class JointQuinticBaseline:
    def __init__(self, robot, q0, target_xyz, target_dir=np.array([0.0, 0.0, -1.0], dtype=np.float32),
        ...
        self.target_dir = safe_unit_np(target_dir)
        self.T_final = float(T_final)
        self.Nt = int(Nt)
        self.dt = self.T_final / (self.Nt - 1)
    ...
    def solve_terminal_q(self, ik_epochs=3000, ik_lr=3e-2):
        solver = TerminalIKSolver(
            self.robot, self.q0, self.target_xyz, target_dir=self.target_dir,
        )
        qT, eeT, tool_z_T, ik_time, ik_history = solver.solve(
            epochs=ik_epochs, lr=ik_lr, print_every=max(1, ik_epochs // 10),
        )
        pos_err = np.linalg.norm(eeT - self.target_xyz)
        ori_err_deg = orientation_angle_deg(tool_z_T, self.target_dir)
        print(
            f"[Quintic-IK] done in {ik_time:.2f} sec | "
            f"final_pos_err={pos_err:.6e} | "
            f"final_ori_err_deg={ori_err_deg:.6e}"
        )
        return qT, tool_z_T, ik_time, ik_history
    ...
    def generate_quintic(self, qT):
        ...
        qdot = (dh_dt * dq).astype(np.float32)
        qddot = (d2h_dt2 * dq).astype(np.float32)
        qjerk = (d3h_dt3 * dq).astype(np.float32)

        _, ee = self.robot.forward_all_points(tf.constant(q, dtype=DTYPE))
        ee = ee.numpy().astype(np.float32)

        _, _, rotT = self.robot.forward_all_points(tf.constant(q[-1:, :], dtype=DTYPE), return_rotation=True)
        final_tool_z_axis = safe_unit_np(rotT[0, :, 2].numpy())

        return q, qdot, qddot, qjerk, ee, final_tool_z_axis

    def run(self, ik_epochs=3000, ik_lr=3e-2):
        t0 = time.time()
        qT, tool_z_T_ik, ik_time, ik_history = self.solve_terminal_q(ik_epochs=ik_epochs, ik_lr=ik_lr)
        q, qdot, qddot, qjerk, ee, final_tool_z_axis = self.generate_quintic(qT)
        ...
        return {
            ...
            "target_dir": self.target_dir.copy(),
            "final_tool_z_axis": final_tool_z_axis,
            "terminal_tool_z_axis_from_ik": tool_z_T_ik,
        }


# ============================================================
# 8) Visualization  (ex1-style: styled axes, publication quality)
# ============================================================
...
def plot_panel_weights(adal, T_final, save_path=None, show=True):
    ...
    fig, axes = plt.subplots(3, 2, figsize=(12, 5.625), sharex=False)
...
def plot_joint_trajectories_compare(t, q_a, q_b, label_a="ADA-L", label_b="Quintic", save_path=None, show=True, q0=None):
    ...
    for j, ax in enumerate(axes):
        ...
        leg_joint = ax.legend(
            [dummy], [joint_labels[j]], loc="best", frameon=False,
...
def plot_ee_paths_compare(ee_a, ee_b, target_xyz, label_a="ADA-L", label_b="Quintic",
    ...
    ax.scatter([target_xyz[0]], [target_xyz[1]], [target_xyz[2]], s=110, marker="*",
...
def save_full_report(
    ...
    # ---- ADA-L single-quantity bundle plots ----
    ...
    plot_robot_snapshots_generic(
        ...
        show=show_figures, xy_center=xy_center,
    )
    ...
    np.savez(
        ...
        ee_adal=adal_res["ee"], final_tool_z_axis_adal=adal_res["final_tool_z_axis"],
        t_quintic=qbase["t"],
        q_quintic=qbase["q"], qdot_quintic=qbase["qdot"],
        qddot_quintic=qbase["qddot"], qjerk_quintic=qbase["qjerk"],
        ee_quintic=qbase["ee"], final_tool_z_axis_quintic=qbase["final_tool_z_axis"],
        q0=run_config["q0"],
        target_xyz=run_config["target_xyz"],
        target_dir=run_config["target_dir"],
    )
    ...
    write_markdown_report(
        ...
        target_xyz=run_config["target_xyz"],
        adal_final_tool_z=adal_res["final_tool_z_axis"],
        quintic_final_tool_z=qbase["final_tool_z_axis"],
        target_dir=run_config["target_dir"],
    )
...
# ============================================================
# 8.5) UR5e trajectory validation (joint limits / velocity / acceleration)
# ============================================================
UR5E_JOINT_NAMES = [
...
if __name__ == "__main__":
    ...
    target_xyz = np.array([0.3, -0.05, 0.65], dtype=np.float32)
    # target_xyz = np.array([0.3, -0.15, 0.55], dtype=np.float32)
    target_dir = np.array([0.0, -1.0, 0.0], dtype=np.float32)

    T_final = 2.0
    Nt = 101

    # xy_center for 3D plot framing: midpoint of start EE and target
    ...
    out_dir = str(Path(__file__).resolve().parent / f"results_adal_vs_quintic_pos_plus_1ori_{timestamp}")
    ...
    adal_metrics = benchmark_metrics(
        ...
        ee=adal_res["ee"], target_xyz=adal_res["target_xyz"],
        dt=T_final / (Nt - 1), solve_time=adal_solve_time,
        final_tool_z_axis=adal_res["final_tool_z_axis"], target_dir=target_dir,
    )
    ...
    quintic = JointQuinticBaseline(
        robot=robot, q0=q0, target_xyz=target_xyz, target_dir=target_dir,
    ...
    quintic_metrics = benchmark_metrics(
        ...
        ee=qbase["ee"], target_xyz=target_xyz,
        dt=T_final / (Nt - 1), solve_time=qbase["solve_time"],
        final_tool_z_axis=qbase["final_tool_z_axis"], target_dir=target_dir,
    )
    ...
    print("\nFinal tool z-axis")
    print("ADA-L final z-axis      :", adal_res["final_tool_z_axis"])
    print("Quintic final z-axis  :", qbase["final_tool_z_axis"])
    ...
    # ---- Joint trajectory CSVs + UR5e validation ----
    ensure_dir(out_dir)
    adal_csv_path     = Path(out_dir) / "joint_trajectory_adal.csv"
    quintic_csv_path = Path(out_dir) / "joint_trajectory_quintic.csv"
    save_joint_trajectory_csv(adal_res["t"], adal_res["q"], adal_csv_path)
    save_joint_trajectory_csv(qbase["t"],   qbase["q"],   quintic_csv_path)

    adal_ok, adal_errs, adal_summary = validate_ur5e_trajectory(adal_res["t"], adal_res["q"])
    qb_ok,  qb_errs,  qb_summary  = validate_ur5e_trajectory(qbase["t"],   qbase["q"])
    print_validation_result("ADA-L",     adal_ok, adal_errs, adal_summary)
    print_validation_result("Quintic", qb_ok,  qb_errs,  qb_summary)

    print("\n=== UR5e Validation Verdict ===")
    print(f"  ADA-L      : {'PASS' if adal_ok else 'FAIL'}  ({adal_csv_path.name})")
    print(f"  Quintic  : {'PASS' if qb_ok  else 'FAIL'}  ({quintic_csv_path.name})")
