...
"""
Benchmark 6 (v3): Payload-Aware Energy Minimization + Collision-Avoidance Loss
- ADA-L multi-waypoint planner with payload-aware torque / power regularization
- B-spline multi-waypoint planner with payload-aware torque / power regularization

v3 adds a TF-differentiable capsule-based collision penalty
(`compute_capsule_collision_loss`) to both planners' compute_loss_train,
weighted by the new `w_collision` constructor argument (default 1e6).
The capsule model and ACM mirror capsule_collision_checker.UR5eCapsuleChecker
(used as the post-hoc validator), so the optimizer steers trajectories
into the same collision-free region the validator certifies.

Both planners use the same PayloadAwareJointDynamics model:
    tau = I_eff(q) * q_ddot + D * q_dot + K * (q - q_ref)
          + payload_gravity_torque(q)

Payload effects are computed via the geometric Jacobian of the
payload centre-of-mass (Siciliano et al. 2009, §3.4):
  (1) Reflected inertia (pose-dependent, diagonal approximation):
        I_payload_i(q) = m * ||J_v_i(q)||^2
        I_eff_i(q)     = I_robot_i + I_payload_i(q)
  (2) Gravity torque (exact for point mass at payload COM):
        tau_gravity = J_v(q)^T * F_g

Coriolis/centrifugal coupling terms are intentionally omitted;
this is a decoupled surrogate used as a differentiable loss proxy.

Multiple payload cases are evaluated in a loop (payload_cases list).
A cross-case summary CSV and bar chart are saved at the root output directory.

Key design:
- align_state() handles length mismatch between ADA-L analytic derivatives
  (same length) and B-spline finite-difference derivatives (shorter by 1-3).
- Loss includes torque_loss, power_loss, tau_limit_loss in addition to
  the standard smoothness / waypoint terms.
- Metrics include integrated_squared_torque, integrated_joint_abs_work, etc.

Formatting follows ex1_v12_kdj_paperfig_csv style:
  ...
  - torque/power comparison figures added (ex6-specific)
  - torque/power metrics added to CSV, markdown, DOCX report
  - payload_case_summary figure and CSV at root level
...
def save_comparison_csv(metrics_a, metrics_b, path, name_a="ADA-L", name_b="B-spline"):
    keys = [
        ...
        "integrated_squared_torque",
        "mean_squared_torque",
        "max_torque_norm",
        "integrated_joint_abs_work",
        "mean_joint_abs_power",
        "max_joint_abs_power",
        "integrated_joint_abs_work",
        "solve_time",
    ]
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for k in keys:
            a = float(metrics_a.get(k, float("nan")))
            b = float(metrics_b.get(k, float("nan")))
            ratio_ab = a / b if abs(b) > 1e-15 else float("nan")
            ratio_ba = b / a if abs(a) > 1e-15 else float("nan")
...
def save_joint_full_csv(t, q, qdot, qddot, qjerk, tau, power_joint, path):
    """Full CSV with time, q (robot convention), qdot, qddot, qjerk, tau, power for all 6 joints."""
    t = np.asarray(t, dtype=np.float32).reshape(-1)
    q = np.asarray(q, dtype=np.float32)
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for i in range(N):
            ...
            row += [float(v) for v in tau[i]]
            row += [float(v) for v in power_joint[i]]
            writer.writerow(row)
...
# ============================================================
# 3-1) Payload-aware dynamics  (Jacobian-based)
#
# Gravity torque and reflected inertia are computed exactly for a
# point-mass payload using the geometric Jacobian of the payload COM.
#
#   J_v_i = z_{i-1} × (p_payload − p_{i-1})     (revolute joint i)
#   τ_gravity = J_v^T · F_g                      (Siciliano et al. 2009, §3.4)
#   I_reflected_ii = m · ‖J_v_i‖²                (diagonal approximation)
#
# Coriolis/centrifugal coupling terms are intentionally omitted.
# ============================================================
class PayloadAwareJointDynamics:
    """
    Payload-aware decoupled joint dynamics surrogate with pose-dependent
    robot mass matrix computed from UR5e manufacturer link parameters.

    Torque model (per joint, diagonal approximation):
        τ = M_diag(q) · q̈ + D · q̇ + K · (q − q_ref) + τ_gravity(q)

    where M_diag(q) includes both the robot's own reflected inertia
    and the payload's reflected inertia, both computed via the
    geometric Jacobian of each body's centre-of-mass.

    Robot mass matrix (diagonal, point-mass model):
        M_robot_ii(q) = Σ_{k≥i} m_k · ‖J_v_i^(k)(q)‖²

    where J_v_i^(k) is the i-th Jacobian column for link k's COM.
    Only joints i ≤ k contribute (distal links only).

    Payload contribution (same Jacobian approach):
        M_payload_ii(q) = m_payload · ‖J_v_i^(payload)(q)‖²

    Gravity torque (exact for point masses):
        τ_gravity = Σ_k  J_v^(k)^T · (m_k · g_vec)
                  + J_v^(payload)^T · (m_payload · g_vec)

    Assumptions:
      - Each link is modelled as a point mass at its COM (rotational
        inertia tensors are neglected; dominant for Link 2 Iyy ≈ 0.85).
      - Off-diagonal inertia coupling is neglected (diagonal approx.).
      - Coriolis/centrifugal terms are omitted (decoupled surrogate).
      - Robot base is mounted upright (base z-axis vertical).

    References
    ----------
    Siciliano et al. (2009). Robotics: Modelling, Planning and
        Control, §3.1 (Geometric Jacobian), §3.5 (Statics). Springer.
    Raviola et al. Electromechanical modeling and identification of
        the UR5 e-series robot. Robotica, Cambridge University Press.
        (UR5e friction coefficients, joint-side after N=100 gear conversion)
    UR5e manufacturer DH/dynamics parameters (Universal Robots).

    Parameters
    ----------
    robot : UR5eKinematics
        Kinematic model (supplies DH parameters and theta_offsets).
    D  : [6] viscous damping per joint (N·m·s/rad)
    K  : [6] stiffness per joint (N·m/rad), typically 0
    q_ref : [6] reference configuration for stiffness term
    tau_limit : [6] joint torque limits (N·m)
    payload_mass : float, payload mass (kg)
    payload_com_local : [3] payload COM in the tool (EE) frame (m)
    gravity_vec : [3] gravity vector in the world (base) frame (m/s²)
    """

    # UR5e manufacturer link parameters
    # Masses [kg] — from UR5e datasheet
    UR5E_LINK_MASS = [3.761, 8.058, 2.846, 1.37, 1.3, 0.365]
    # COM positions in each link's own frame [m] — from UR5e datasheet
    UR5E_LINK_COM = [
    ...
    def __init__(
        self,
        robot,
        D=None, K=None, q_ref=None, tau_limit=None,
        payload_mass=0.0, payload_com_local=None,
        gravity_vec=None,
    ):
        self.robot = robot

        # Link masses and COM (class constants → tensors)
        self.link_mass = [
            tf.constant(m, dtype=DTYPE)
            for m in self.UR5E_LINK_MASS
        ]
        self.link_com = [
            tf.constant(np.array(c, dtype=np.float32), dtype=DTYPE)
            for c in self.UR5E_LINK_COM
        ]

        # Viscous damping — joint-side, identified for UR5e (e-series).
        # D_joint = N^2 * k_v_motor with harmonic-drive ratio N = 100;
        # k_v_motor [Nm·s/rad] = [0.475, 1.073, 0.382, 0.295, 0.114, 0.188]e-3
        # (Raviola et al., "Electromechanical modeling and identification of
        #  the UR5 e-series robot", Robotica, Cambridge, Table VI).
        self.D = tf.constant(
            np.array(D if D is not None else
                     [4.75, 10.73, 3.82, 2.95, 1.14, 1.88], dtype=np.float32),
            dtype=DTYPE)
        self.K = tf.constant(
            np.array(K if K is not None else [0.0]*6, dtype=np.float32),
            dtype=DTYPE)
        self.q_ref = tf.constant(
            np.array(q_ref if q_ref is not None else [0.0]*6,
                     dtype=np.float32),
            dtype=DTYPE)
        self.tau_limit = tf.constant(
            np.array(tau_limit if tau_limit is not None else
                     [150, 150, 150, 28, 28, 28], dtype=np.float32),
            dtype=DTYPE)
        self.payload_mass = tf.constant(float(payload_mass), dtype=DTYPE)
        self.payload_com_local = tf.constant(
            np.array(payload_com_local if payload_com_local is not None
                     else [0.0, 0.0, 0.15], dtype=np.float32),
            dtype=DTYPE)
        self.gravity_vec = tf.constant(
            np.array(gravity_vec if gravity_vec is not None
                     else [0.0, 0.0, -9.81], dtype=np.float32),
            dtype=DTYPE)

    def align_state(self, q, qdot, qddot):
        """
        Align q, qdot, qddot onto a common time grid.

        ADA-L analytic: all same length Nt          -> no change.
        B-spline FD:  q[Nt], qdot[Nt-1], qddot[Nt-2] -> align to Nt-2.

        Strategy: take the last min_len rows from each array.
        """
        nq = tf.shape(q)[0]
        nv = tf.shape(qdot)[0]
        na = tf.shape(qddot)[0]
        min_len = tf.reduce_min([nq, nv, na])
        return (q[nq - min_len:],
                qdot[nv - min_len:],
                qddot[na - min_len:])

    def _compute_fk_frames(self, q):
        """
        Run forward kinematics and store per-frame z-axes, origins,
        and rotation matrices needed for Jacobian computation.

        Returns
        -------
        z_axes   : list of 7 tensors [N, 3]  (z_0 … z_6)
        origins  : list of 7 tensors [N, 3]  (p_0 … p_6)
        rotmats  : list of 7 tensors [N, 3, 3]  (R_0 … R_6)
        """
        rob = self.robot
        N = tf.shape(q)[0]
        T = tf.eye(4, batch_shape=[N], dtype=DTYPE)
        ...
        for i in range(6):
            theta_i = q[:, i] + tf.constant(rob.theta_offset[i], dtype=DTYPE)
            T = tf.matmul(
                T,
                dh_transform(rob.alpha[i], rob.a[i], rob.d[i], theta_i))
            z_axes.append(T[:, :3, 2])
            origins.append(T[:, :3, 3])
            rotmats.append(T[:, :3, :3])

        return z_axes, origins, rotmats

    def _compute_mass_matrix_and_gravity(self, q):
        """
        Compute the full pose-dependent diagonal mass matrix and
        gravity torque for the robot links AND the payload.

        Mass matrix diagonal (point-mass model):
            M_ii(q) = Σ_{k≥i} m_k · ‖z_{i-1} × (p_com_k − p_{i-1})‖²
                    + m_payload · ‖z_{i-1} × (p_payload − p_{i-1})‖²

        Gravity torque (= ∂P/∂q where P = -Σ m_l · g_vec · r_com_l, height-based PE):
            τ_g_i(q) = -Σ_k [z_{i-1} × (p_com_k − p_{i-1})] · (m_k · g_vec)
                      - [z_{i-1} × (p_payload − p_{i-1})] · (m_payload · g_vec)

        With g_vec = (0, 0, -9.81), this gives positive τ_g when motors must
        push UP against gravity. EOM convention: τ_motor = M·q̈ + ... + τ_g.

        Only joints i ≤ k contribute to link k (distal-only rule).

        Returns
        -------
        M_diag  : [N, 6]  pose-dependent diagonal mass matrix
        tau_g   : [N, 6]  total gravity torque (robot + payload)
        """
        z_axes, origins, rotmats = self._compute_fk_frames(q)
        N = tf.shape(q)[0]

        M_diag = tf.zeros([N, 6], dtype=DTYPE)
        tau_g  = tf.zeros([N, 6], dtype=DTYPE)
        g_vec  = self.gravity_vec                          # [3]

        # ── Robot links ───────────────────────────────────
        for k in range(6):
            # Link k's COM in world frame
            #   p_com_k = p_{k+1} + R_{k+1} · com_local_k
            # where frame k+1 is the frame after joint k+1's DH transform.
            R_k = rotmats[k + 1]                           # [N, 3, 3]
            p_k = origins[k + 1]                           # [N, 3]
            p_com = p_k + tf.einsum(
                'nij,j->ni', R_k, self.link_com[k])       # [N, 3]
            m_k = self.link_mass[k]
            F_g_k = m_k * g_vec                            # [3]

            # Accumulate Jacobian contributions for joints 0..k
            for i in range(k + 1):
                r = p_com - origins[i]                     # [N, 3]
                J_vi = tf.linalg.cross(z_axes[i], r)      # [N, 3]

                # Mass: m_k · ‖J_vi‖²
                M_contrib = m_k * tf.reduce_sum(
                    tf.square(J_vi), axis=1)               # [N]
                # Gravity (∂P/∂q_i): -J_vi · F_g_k
                tau_contrib = -tf.reduce_sum(
                    J_vi * F_g_k[None, :], axis=1)         # [N]

                # Scatter-add into joint i
                idx = tf.constant(i, dtype=tf.int32)
                M_diag  = M_diag  + tf.one_hot(idx, 6, dtype=DTYPE)[None, :] * M_contrib[:, None]
                tau_g   = tau_g   + tf.one_hot(idx, 6, dtype=DTYPE)[None, :] * tau_contrib[:, None]

        # ── Payload ───────────────────────────────────────
        R_ee = rotmats[6]                                  # [N, 3, 3]
        p_ee = origins[6]                                  # [N, 3]
        p_payload = p_ee + tf.einsum(
            'nij,j->ni', R_ee, self.payload_com_local)     # [N, 3]
        F_g_payload = self.payload_mass * g_vec             # [3]

        for i in range(6):
            r = p_payload - origins[i]                      # [N, 3]
            J_vi = tf.linalg.cross(z_axes[i], r)           # [N, 3]

            M_contrib = self.payload_mass * tf.reduce_sum(
                tf.square(J_vi), axis=1)                    # [N]
            tau_contrib = -tf.reduce_sum(
                J_vi * F_g_payload[None, :], axis=1)        # [N]
    ...
    def torque(self, q, qdot, qddot):
        ...
        where M_diag(q) is the pose-dependent diagonal mass matrix
        including both robot links and payload, and τ_gravity includes
        gravity contributions from all robot links and the payload.
        """
        q_use, qdot_use, qddot_use = self.align_state(q, qdot, qddot)

        # Pose-dependent mass matrix + gravity (single FK pass)
        M_diag, tau_g = self._compute_mass_matrix_and_gravity(q_use)

        tau = (M_diag * qddot_use
               + self.D[None, :] * qdot_use
               + self.K[None, :] * (q_use - self.q_ref[None, :])
               + tau_g)
        return tau

    def power(self, q, qdot, qddot):
        q_use, qdot_use, qddot_use = self.align_state(q, qdot, qddot)
        tau = self.torque(q_use, qdot_use, qddot_use)
        power_joint = tau * qdot_use
        power_total = tf.reduce_sum(power_joint, axis=1)
        return tau, power_joint, power_total, q_use, qdot_use, qddot_use
...
def build_derivative_chain(u_grid, n_ctrl, degree):
    """Build B-spline analytic-derivative chain.

    Returns (B_q, D1, B_qd, D2, B_qdd, D3, B_qjerk) so that
        q     = B_q @ ctrl
        qd_u  = B_qd @ (D1 @ ctrl)
        qdd_u = B_qdd @ (D2 @ D1 @ ctrl)
        qj_u  = B_qjerk @ (D3 @ D2 @ D1 @ ctrl)
    All in u-domain; convert to t by /T^k.
    """
...
# ============================================================
# 4c) Capsule-based differentiable collision loss
#
# Mirrors capsule_collision_checker.UR5eCapsuleChecker but in pure TF so
# the optimizer can take gradients of the violation against q. Each
# UR5e link is approximated as a capsule (segment + radius); the payload
# is an OBB attached to tool0. For every pair of capsules (and each
# capsule vs payload) we sample the central segment, compute distances,
# and sum a squared ReLU(threshold - distance) penalty across timesteps.
# Pairs in the standard UR5e SRDF "Adjacent"/"Never" list are skipped.
# ============================================================
_COLL_R_BASE      = 0.080
_COLL_R_SHOULDER  = 0.075
_COLL_R_UPPER_ARM = 0.075
_COLL_R_FOREARM   = 0.062
_COLL_R_WRIST_1   = 0.060
_COLL_R_WRIST_2   = 0.060
_COLL_R_WRIST_3   = 0.055

_COLL_SKIP_PAIRS = frozenset({
    ("base",      "shoulder"),
    ("shoulder",  "upper_arm"),
    ("upper_arm", "forearm"),
    ("forearm",   "wrist_1"),
...
def _coll_pair_skipped(a, b):
    return (a, b) in _COLL_SKIP_PAIRS or (b, a) in _COLL_SKIP_PAIRS


def _point_obb_dist_tf(p, center, axes, half_extents):
    """
    Distance from point p [N, 3] to OBB(center [N, 3], axes [N, 3, 3] with
    columns being box-frame basis, half_extents [3]). Returns [N], 0 inside.
    """
    rel  = p - center
    proj = tf.einsum("nk,nki->ni", rel, axes)   # rel . axes[:, :, i]
    h    = tf.cast(half_extents, p.dtype)
    excess = tf.maximum(tf.abs(proj) - h, tf.cast(0.0, p.dtype))
    return tf.norm(excess, axis=-1)


def _seg_seg_dist_tf(p0a, p1a, p0b, p1b, eps=1e-12):
    """
    Batched segment-segment closest distance (Eberly). Inputs are [N, 3]
    tensors of segment endpoints; returns [N] distances. Mirrors the numpy
    `_segment_segment_distance` in capsule_collision_checker.py so the
    training-time loss uses the SAME distance the validator checks.
    UR5e segments are always non-degenerate (a, e > 0), so the a<=eps /
    e<=eps degenerate branches are not implemented; we just clamp denom
    away from zero with `eps` for numerical safety.
    """
    dtype = p0a.dtype
    ...
    d2 = p1b - p0b
    r  = p0a - p0b
    a  = tf.reduce_sum(d1 * d1, axis=-1)
    e  = tf.reduce_sum(d2 * d2, axis=-1)
    f  = tf.reduce_sum(d2 * r,  axis=-1)
    c  = tf.reduce_sum(d1 * r,  axis=-1)
    b  = tf.reduce_sum(d1 * d2, axis=-1)
    denom = a * e - b * b
    safe_denom = tf.where(tf.abs(denom) > eps_t, denom, tf.ones_like(denom) * eps_t)
    s_default = tf.clip_by_value((b * f - c * e) / safe_denom,
                                 tf.cast(0.0, dtype), tf.cast(1.0, dtype))
    s_default = tf.where(tf.abs(denom) > eps_t, s_default, tf.zeros_like(s_default))
    safe_e = tf.maximum(e, eps_t)
    safe_a = tf.maximum(a, eps_t)
    t_unc  = (b * s_default + f) / safe_e
    s_tneg = tf.clip_by_value(-c / safe_a,
                              tf.cast(0.0, dtype), tf.cast(1.0, dtype))
    s_tpos = tf.clip_by_value((b - c) / safe_a,
                              tf.cast(0.0, dtype), tf.cast(1.0, dtype))
    s = tf.where(t_unc < tf.cast(0.0, dtype), s_tneg,
        tf.where(t_unc > tf.cast(1.0, dtype), s_tpos, s_default))
    t = tf.clip_by_value(t_unc, tf.cast(0.0, dtype), tf.cast(1.0, dtype))
    closest1 = p0a + s[:, None] * d1
    closest2 = p0b + t[:, None] * d2
    return tf.norm(closest1 - closest2, axis=-1)


def compute_capsule_collision_loss(q_traj, robot,
                                   payload_size=(0.15, 0.15, 0.0175),
                                   payload_offset_z=None,
                                   safety_margin=0.005,
                                   n_samples=12):
    """
    Differentiable UR5e self + payload OBB collision penalty, aligned with
    `capsule_collision_checker.UR5eCapsuleChecker`:
      - capsule-vs-capsule uses exact Eberly segment-segment distance
        (`_seg_seg_dist_tf`) so the training distance equals the validator's.
      - capsule-vs-OBB uses the same 12-sample scheme as the validator.
      - hinge is squared ReLU. The exact distance term is what removes the
        sampling-overestimate bias; squaring keeps the loss C^1 smooth so
        L-BFGS doesn't get trapped at the violation boundary kink.

    Args:
        q_traj: [N, 6] tensor of DH-convention joint angles.
        robot:  UR5eKinematics instance (provides DH parameters).
        payload_size: (sx, sy, sz) box dimensions in tool0 local frame.
        payload_offset_z: distance from tool0 origin to box center along
                          tool0 +Z. Defaults to payload_size[2] / 2.
        safety_margin: buffer (m) added to capsule radii so the squared
                       penalty has enough boundary force to push trajectories
                       cleanly outside the validator's geometric collision
                       region. 5 mm balances optimizer stability and feasible
                       region size.
        n_samples: number of points sampled along each capsule for the
                   capsule-vs-payload-OBB distance (matches validator's 12).

    Returns:
        Scalar tensor — mean of squared violations, summed across all
        active pairs and averaged over timesteps.
    """
    q = tf.cast(q_traj, DTYPE)
    if payload_offset_z is None:
        payload_offset_z = float(payload_size[2]) / 2.0

    # ---- FK: build all 7 SE(3) transforms per timestep ----
    N = tf.shape(q)[0]
    T = tf.tile(tf.eye(4, dtype=DTYPE)[None], [N, 1, 1])
    Ts = [T]
    for i in range(6):
        theta_i = q[:, i] + tf.constant(robot.theta_offset[i], dtype=DTYPE)
        T = tf.matmul(
            T, dh_transform(robot.alpha[i], robot.a[i], robot.d[i], theta_i)
        )
        Ts.append(T)

    P      = [Ti[:, :3, 3] for Ti in Ts]   # 7 tensors of [N, 3]
    R_tool = Ts[6][:, :3, :3]              # [N, 3, 3]
    z1     = Ts[1][:, :3, 2]               # [N, 3] — joint-1 z axis

    # Static base capsule (broadcast over time)
    base_p0 = tf.broadcast_to(tf.constant([0.0, 0.0, 0.0],  dtype=DTYPE)[None, :], [N, 3])
    base_p1 = tf.broadcast_to(tf.constant([0.0, 0.0, 0.10], dtype=DTYPE)[None, :], [N, 3])

    shoulder_p0 = P[1] - tf.cast(0.04, DTYPE) * z1
    shoulder_p1 = P[1] + tf.cast(0.04, DTYPE) * z1

    capsules = [
        ("base",      base_p0,     base_p1,     _COLL_R_BASE),
        ("shoulder",  shoulder_p0, shoulder_p1, _COLL_R_SHOULDER),
        ("upper_arm", P[1],        P[2],        _COLL_R_UPPER_ARM),
        ("forearm",   P[2],        P[3],        _COLL_R_FOREARM),
        ("wrist_1",   P[3],        P[4],        _COLL_R_WRIST_1),
        ("wrist_2",   P[4],        P[5],        _COLL_R_WRIST_2),
        ("wrist_3",   P[5],        P[6],        _COLL_R_WRIST_3),
    ]

    # Payload OBB: center at flange + offset_z along tool0 +Z
    payload_center = P[6] + R_tool[:, :, 2] * tf.cast(payload_offset_z, DTYPE)
    payload_he = tf.constant(
        [payload_size[0] / 2.0, payload_size[1] / 2.0, payload_size[2] / 2.0],
        dtype=DTYPE,
    )

    ts = tf.linspace(tf.cast(0.0, DTYPE), tf.cast(1.0, DTYPE), n_samples)  # [n]
    ts = ts[None, :, None]                     # [1, n, 1]
    margin = tf.cast(safety_margin, DTYPE)
    total = tf.cast(0.0, DTYPE)

    # ---- capsule-capsule pairs (validator-equivalent: exact Eberly distance) ----
    n_caps = len(capsules)
    for i in range(n_caps):
        ni, p0i, p1i, ri = capsules[i]
        for j in range(i + 1, n_caps):
            nj, p0j, p1j, rj = capsules[j]
            if _coll_pair_skipped(ni, nj):
                continue
            d = _seg_seg_dist_tf(p0i, p1i, p0j, p1j)                   # [N]
            threshold = margin + tf.cast(ri + rj, DTYPE)
            violation = tf.nn.relu(threshold - d)
            total = total + tf.reduce_mean(tf.square(violation))

    # ---- capsule-vs-payload-OBB pairs ----
    for ni, p0i, p1i, ri in capsules:
        if _coll_pair_skipped(ni, "payload"):
            continue
        samples = p0i[:, None, :] + ts * (p1i - p0i)[:, None, :]       # [N, n, 3]
        samples_flat = tf.reshape(samples, [-1, 3])                    # [N*n, 3]
        center_rep = tf.repeat(payload_center, n_samples, axis=0)      # [N*n, 3]
        axes_rep   = tf.repeat(R_tool,         n_samples, axis=0)      # [N*n, 3, 3]
        d_flat = _point_obb_dist_tf(samples_flat, center_rep, axes_rep, payload_he)
        d_per  = tf.reshape(d_flat, [-1, n_samples])
        d_min  = tf.reduce_min(d_per, axis=1)
        threshold = margin + tf.cast(ri, DTYPE)
        violation = tf.nn.relu(threshold - d_min)
        total = total + tf.reduce_mean(tf.square(violation))

    # ---- capsule-vs-floor (z=0 plane), validator-aligned ----
    # Lowest point of capsule = min(p0.z, p1.z) - r. Skip "base" (pedestal).
    for ni, p0i, p1i, ri in capsules:
        if ni == "base":
            continue
        min_z = tf.minimum(p0i[:, 2], p1i[:, 2])           # [N]
        threshold_floor = margin + tf.cast(ri, DTYPE)
        violation = tf.nn.relu(threshold_floor - min_z)    # fires when min_z < margin + r
        total = total + tf.reduce_mean(tf.square(violation))

    # ---- payload-OBB-vs-floor ----
    # Lowest z of OBB = center.z - sum_j half_extent_j * |axis_j.z|.
    abs_axis_z_world = tf.abs(R_tool[:, 2, :])             # [N, 3]
    half_z_extent    = tf.reduce_sum(abs_axis_z_world * payload_he[None, :], axis=-1)  # [N]
    min_z_payload    = payload_center[:, 2] - half_z_extent
    violation = tf.nn.relu(margin - min_z_payload)
    total = total + tf.reduce_mean(tf.square(violation))
...
# ============================================================
# 5) ADA-L multi-waypoint planner  (dynamic-aware)
# ============================================================
class ADALMultiWaypointPlanner:
    def __init__(
        ...
        payload_mass=0.0, payload_com_local=(0.0, 0.0, 0.15),
        z_floor=0.0,
        # ---- collision-loss knobs (v3) ----
        payload_size=(0.15, 0.15, 0.0175),
        payload_offset_z=None,
        w_collision=1.0e6,
        coll_safety_margin=0.005,
        coll_n_samples=12,
    ):
        ...
        self.payload_com_local = np.asarray(payload_com_local, dtype=np.float32).reshape(3,)

        # Collision-loss config
        self.payload_size       = tuple(float(s) for s in payload_size)
        self.payload_offset_z   = (
            float(self.payload_size[2]) / 2.0
            if payload_offset_z is None else float(payload_offset_z)
        )
        self.w_collision        = float(w_collision)
        self.coll_safety_margin = float(coll_safety_margin)
        self.coll_n_samples     = int(coll_n_samples)

        # Create robot FIRST so it can be passed to dynamics
        self.robot = UR5eKinematics()

        self.dyn = PayloadAwareJointDynamics(
            robot=self.robot,
            D=[4.75, 10.73, 3.82, 2.95, 1.14, 1.88],
            K=[0.0]*6,
            q_ref=self.q0,
            tau_limit=[150, 150, 150, 60, 60, 60],
            payload_mass=self.payload_mass,
            payload_com_local=self.payload_com_local,
        ...
        self.history = {
            "loss": [], "waypoint": [], "path": [], "acc": [], "jerk": [],
            "limit": [], "final_vel": [], "final_acc": [], "init_acc": [],
            "torque": [], "power": [], "tau_limit": [], "collision": [],
        }
    ...
    def compute_loss_train(self):
        ...
        # Dynamic-aware terms
        tau = self.dyn.torque(q, qdot, qddot)
        power_joint = tau * qdot          # ADA-L: same length, align_state is no-op
        power_total = tf.reduce_sum(power_joint, axis=1)
        torque_loss   = tf.reduce_mean(tf.square(tau))
        power_loss    = tf.reduce_mean(tf.square(power_joint))
        tau_violation = tf.nn.relu(tf.abs(tau) - self.dyn.tau_limit[None, :])
        tau_limit_loss = tf.reduce_mean(tf.square(tau_violation))
        ...
        # ---- v3 collision penalty (capsule-based, differentiable) ----
        collision_loss = compute_capsule_collision_loss(
            q, self.robot,
            payload_size=self.payload_size,
            payload_offset_z=self.payload_offset_z,
            safety_margin=self.coll_safety_margin,
            n_samples=self.coll_n_samples,
        )

        # V2 Phase 2 best ADA-L weights: jerk=130, w_t=2, w_p=3
        loss = (
            50000000.0 * waypoint_loss
            + 1 * path_loss
            # + 1.0 * acc_loss
            + 1.0 * jerk_loss
            + 1 * torque_loss
            ...
            + 1000.0 * floor_loss
            + tf.cast(self.w_collision, DTYPE) * collision_loss
        )
        ...
        return {
            ...
            "torque": torque_loss, "power": power_loss, "tau_limit": tau_limit_loss,
            "floor": floor_loss, "collision": collision_loss,
            "q": q, "qdot": qdot, "qddot": qddot, "qjerk": qjerk,
            "ee": ee, "ee_wp": ee_wp, "tau_all": tau_all,
            "tau_dyn": tau, "power_total": power_total, "power_joint": power_joint,
        }
    ...
    def run_lbfgs(self, maxiter=2000):
        ...
        print(
            ...
            f"torque={out['torque'].numpy():.6e} | power={out['power'].numpy():.6e} | "
    ...
    def train_step(self):
        ...
        return (loss, out["waypoint"], out["path"], out["acc"], out["jerk"], out["limit"],
                out["final_vel"], out["final_acc"], out["init_acc"], out["torque"], out["power"], out["tau_limit"])

    def train(self, epochs=1200, lr=3e-2, print_every=200, use_lbfgs=True, lbfgs_maxiter=2000):
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=lr)
        t0 = time.time()
        for ep in range(1, epochs + 1):
            loss, wp, path, acc, jerk, limit, fvel, facc, iacc, torque, power, tau_lim = self.train_step()
            ...
            self.history["torque"].append(float(torque.numpy()))
            self.history["power"].append(float(power.numpy()))
            self.history["tau_limit"].append(float(tau_lim.numpy()))
            if ep % print_every == 0 or ep == 1:
                ...
                print(
                    ...
                    f"torque={torque.numpy():.6e} | power={power.numpy():.6e} | "
...
class BSplineMultiWaypointPlanner:
    def __init__(
        ...
        #  first 3 ctrl pts == q0 (fixed) -> q(0)=q0, qd(0)=0, qdd(0)=0
        #  last  3 ctrl pts == qT_var (single learnable vec, tiled 3x) -> qd(T)=0, qdd(T)=0
        #  middle (n_ctrl-6) ctrl pts are free
        if self.n_ctrl < 7:
    ...
    def q_traj_with_derivatives(self):
        ctrl = self.assemble_ctrl()                      # [n_ctrl, 6]
    ...
    def compute_loss_train(self):
        ...
        # Dynamic-aware terms (analytic derivatives have full length, no align needed)
        tau = self.dyn.torque(q, qdot, qddot)
        power_joint = tau * qdot
        power_total = tf.reduce_sum(power_joint, axis=1)
        torque_loss   = tf.reduce_mean(tf.square(tau))
        ...
        # V2 Phase 2 best BS weights: jerk=5000 (ADA-L jerk x ~38), w_t=2, w_p=3
        loss = (
            50000000.0 * waypoint_loss
            + 1 * path_loss
            # + 1.0 * acc_loss
    ...
    def train_step(self):
        ...
        return (loss, out["waypoint"], out["start"], out["path"], out["acc"], out["jerk"],
                out["limit"], out["final_vel"], out["final_acc"], out["init_acc"],
                out["torque"], out["power"], out["tau_limit"])

    def train(self, epochs=500, lr=3e-2, print_every=200, use_lbfgs=True, lbfgs_maxiter=2000):
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=lr)
        t0 = time.time()
        for ep in range(1, epochs + 1):
            loss, wp, start, path, acc, jerk, limit, fvel, facc, iacc, torque, power, tau_lim = self.train_step()
            self.history["loss"].append(float(loss.numpy()))
            self.history["waypoint"].append(float(wp.numpy()))
    ...
    def results(self):
        ...
        return {
            ...
            "ee": ee.numpy(), "ee_wp": ee_wp.numpy(),
            "t_q":    self.t_np.copy(),
            "t_qdot": self.t_np.copy(),
            "t_qddot": self.t_np.copy(),
            "t_qjerk": self.t_np.copy(),
            "t_tau":   self.t_np.copy(),
            "t_power": self.t_np.copy(),
            "waypoints_xyz": self.waypoints_xyz.copy(),
            "waypoint_times_init": self.waypoint_times.copy(),
            "waypoint_times_opt": tau_all.numpy(),
            "payload_mass": self.payload_mass,
...
def benchmark_multiwaypoint_metrics(res, solve_time):
    ...
    power_joint = res.get("power_joint", None)
    ...
    metrics = {
        ...
        "payload_mass":      float(res.get("payload_mass", 0.0)),
        "payload_com_local": np.asarray(res.get("payload_com_local", np.zeros(3, dtype=np.float32))).copy(),
    }

    if tau is not None:
        dt_tau = T_total / (len(tau) - 1) if len(tau) > 1 else 0.0
        tau_norm_t = np.linalg.norm(tau, axis=1)
        metrics.update({
            "integrated_squared_torque": float(np.sum(np.sum(tau**2, axis=1)) * dt_tau),
            "mean_squared_torque":       float(np.mean(np.sum(tau**2, axis=1))),
            "max_torque_norm":           float(np.max(tau_norm_t)),
            "max_abs_torque_each_joint": np.max(np.abs(tau), axis=0),
        })

    if power_joint is not None:
        dt_powj = T_total / (len(power_joint) - 1) if len(power_joint) > 1 else 0.0
        joint_abs_per_t = np.sum(np.abs(power_joint), axis=1)   # [Nt] total joint abs power per t
        metrics.update({
            "integrated_joint_abs_work": float(np.sum(joint_abs_per_t) * dt_powj),
            "mean_joint_abs_power":      float(np.mean(joint_abs_per_t)),
            "max_joint_abs_power":       float(np.max(joint_abs_per_t)),
        })
...
def print_comparison_table(metrics_a, metrics_b, name_a="ADA-L", name_b="B-spline"):
    ...
    print("=" * 108)
    for k in keys:
        a = metrics_a.get(k, float("nan"))
        b = metrics_b.get(k, float("nan"))
        print(f"{k:>36s} | {a:18.6e} | {b:18.6e}")
    print("=" * 108)
...
def write_markdown_report_multiwaypoint(
    ...
    lines = []
    lines.append(f"# {name_a} vs {name_b} Payload-Aware Energy Minimization Report\n")
    ...
    lines.append(f"- payload_mass: `{run_config.get('payload_mass', 0.0)}`")
    lines.append(f"- payload_com_local: `{np.array2string(np.asarray(run_config.get('payload_com_local', [0,0,0])), precision=5)}`\n")
    ...
    for k in keys:
        a = metrics_a.get(k, float("nan"))
        b = metrics_b.get(k, float("nan"))
        lines.append(f"| {k} | {float(a):.6e} | {float(b):.6e} |")
    lines.append("")
...
def try_write_docx_report_multiwaypoint(
    ...
    doc = Document()
    doc.add_heading(f"{name_a} vs {name_b} Payload-Aware Energy Minimization Report", level=0)
    ...
    doc.add_paragraph(f"payload_mass: {run_config.get('payload_mass', 0.0)}")
    doc.add_paragraph(f"payload_com_local: {np.array2string(np.asarray(run_config.get('payload_com_local', [0,0,0])), precision=5)}")
    ...
    for k in keys:
        ...
        row[0].text = k
        row[1].text = f"{float(metrics_a.get(k, float('nan'))):.6e}"
        row[2].text = f"{float(metrics_b.get(k, float('nan'))):.6e}"
...
# ============================================================
# 9) Visualization  (ex1-style axes + ex6-specific content)
# ============================================================
...
def plot_robot_snapshots_generic(robot, q, waypoints_xyz=None, waypoint_times=None,
                                  time_grid=None, n_show=5, title="Robot snapshots",
                                  save_path=None, show=False):
...
def save_full_report_multiwaypoint(
    ...
    plot_joint_bundle_single(
        adal_res["t_tau"], adal_res["tau"],
        title="ADA-L joint torque",
        y_label="Torque,\n$\\tau$ (N·m)",
        labels=joint_labels, wp_t=wp_t_adal,
        save_path=add_fig("ADA-L joint torque", "adal_joint_torque.png"),
        show=show_figures,
    )
    ...
    adal_tau_norm   = np.linalg.norm(adal_res["tau"],   axis=1)
    bs_tau_norm    = np.linalg.norm(bs_res["tau"],    axis=1)
    ...
    plot_norm_profiles_compare(
        adal_res["t_tau"], adal_tau_norm, bs_tau_norm,
        title="Joint torque norm: ADA-L vs B-spline",
        y_label="Torque norm, \n$\\|\\tau\\|$ (N·m)",
        label_a="ADA-L", label_b="B-spline", wp_t=wp_t_adal,
        save_path=add_fig("Torque norm comparison", "compare_torque_norm.png"),
        show=show_figures,
    )
    plot_norm_profiles_compare(
        adal_res["t_power"],
        np.sum(np.abs(adal_res["power_joint"]), axis=1),
        np.sum(np.abs(bs_res["power_joint"]),  axis=1),
        title="Total joint absolute power comparison",
        y_label=r"$\sum_i |\tau_i\dot q_i|$ (W)",
        label_a="ADA-L", label_b="B-spline", wp_t=wp_t_adal,
        save_path=add_fig("Total joint abs power comparison", "compare_joint_abs_power.png"),
        show=show_figures,
    )
    ...
    save_joint_trajectory_csv(adal_res["t_q"], adal_res["q"],
                               Path(out_dir) / "joint_trajectory_adal.csv")
    save_joint_trajectory_csv(bs_res["t_q"],  bs_res["q"],
                               Path(out_dir) / "joint_trajectory_bspline.csv")

    # Full trajectory CSVs (q, qdot, qddot, qjerk, tau, power) for all 6 joints
    save_joint_full_csv(
        adal_res["t_q"], adal_res["q"], adal_res["qdot"],
        adal_res["qddot"], adal_res["qjerk"],
        adal_res["tau"], adal_res["power_joint"],
        Path(out_dir) / "ADAL_trajectory_full.csv")
    save_joint_full_csv(
        bs_res["t_q"], bs_res["q"], bs_res["qdot"],
        bs_res["qddot"], bs_res["qjerk"],
        bs_res["tau"], bs_res["power_joint"],
    ...
    np.savez(
        ...
        payload_mass=run_config.get("payload_mass", 0.0),
        q_adal=adal_res["q"],    qdot_adal=adal_res["qdot"],
        qddot_adal=adal_res["qddot"], qjerk_adal=adal_res["qjerk"],
        tau_adal=adal_res["tau"], power_joint_adal=adal_res["power_joint"],
        ee_adal=adal_res["ee"],  ee_wp_adal=adal_res["ee_wp"],
        waypoint_times_opt_adal=adal_res["waypoint_times_opt"],
        q_bs=bs_res["q"],      qdot_bs=bs_res["qdot"],
        qddot_bs=bs_res["qddot"],   qjerk_bs=bs_res["qjerk"],
        tau_bs=bs_res["tau"],  power_joint_bs=bs_res["power_joint"],
        ee_bs=bs_res["ee"],    ee_wp_bs=bs_res["ee_wp"],
        waypoint_times_opt_bs=bs_res["waypoint_times_opt"],
    )
...
# ============================================================
# 11) Payload case summary plot  (ex6-specific)
# ============================================================
def plot_payload_case_summary(all_case_rows, save_path=None, show=False):
    """Bar chart comparing integrated joint absolute work across payload cases."""
    case_names = [r["case_name"] for r in all_case_rows]
    adal_work   = [r["adal_integrated_joint_abs_work"] for r in all_case_rows]
    bs_work    = [r["bs_integrated_joint_abs_work"]  for r in all_case_rows]
    adal_torque = [r["adal_integrated_squared_torque"] for r in all_case_rows]
    bs_torque  = [r["bs_integrated_squared_torque"]  for r in all_case_rows]
    ...
    ax = axes[0]
    ax.bar(x - w/2, adal_work, width=w, label="ADA-L")
    ax.bar(x + w/2, bs_work,  width=w, label="B-spline")
    ax.set_xticks(x)
    ax.set_xticklabels(case_names, rotation=15, ha="right", fontsize=11)
    ax.set_ylabel("Integrated joint abs work (J)")
    ax.set_title("Payload-aware energy comparison")
    style_2d_axes(ax)
    ax.legend(frameon=False)

    ax = axes[1]
    ax.bar(x - w/2, adal_torque, width=w, label="ADA-L")
    ax.bar(x + w/2, bs_torque,  width=w, label="B-spline")
    ax.set_xticks(x)
    ax.set_xticklabels(case_names, rotation=15, ha="right", fontsize=11)
    ax.set_ylabel("Integrated torque² (N²·m²·s)")
    ax.set_title("Payload-aware torque comparison")
    style_2d_axes(ax)
    ax.legend(frameon=False)
...
# ============================================================
# 11.5) UR5e trajectory validation (joint limits / velocity / acceleration)
# ============================================================
UR5E_JOINT_NAMES = [
...
def validate_collision(q_traj, payload_size=(0.15, 0.15, 0.0175)):
    """
    Check the entire trajectory for UR5e self / payload collision using
    capsule_collision_checker.UR5eCapsuleChecker. Returns (ok, first_idx,
    n_collide, total). `ok=True` iff no state in the trajectory collides.
    """
    from capsule_collision_checker import UR5eCapsuleChecker
    ck = UR5eCapsuleChecker(payload_size=payload_size)
...
def print_collision_result(name, ok, first_idx, n_coll, n_total, t_traj=None,
                           q_first=None, payload_size=(0.15, 0.15, 0.0175)):
    print(f"\n=== {name} collision check ===")
    if ok:
        print(f"  [OK] No collision in {n_total} states "
              f"(payload size = {payload_size[0]:.3f} x {payload_size[1]:.3f} x "
              f"{payload_size[2]:.4f} m)")
        return
    t_str = ""
...
if __name__ == "__main__":
    ...
    q0 = (q0_robot - _theta_offset).astype(np.float32)
    ...
    root_out_dir = str(Path(__file__).resolve().parent / f"results_payload_energy_{timestamp}")
    ensure_dir(root_out_dir)

    payload_cases = [
        {
            "name": "payload_3kg_centered",
            "payload_mass": 3,
            "payload_com_local": [0.0, 0.0, 0.0875],
        },
        # {
        #     "name": "payload_5kg_centered",
        #     "payload_mass": 3.0,
        #     "payload_com_local": [0.0, 0.0, 0.15],
        # },
        # {
        #     "name": "payload_5kg_offset",
        #     "payload_mass": 3.0,
        #     "payload_com_local": [0.15, 0.0, 0.15],
        # },
    ]
    ...
    for case in payload_cases:
        ...
        payload_com_local = np.asarray(case["payload_com_local"], dtype=np.float32)

        out_dir = os.path.join(root_out_dir, case_name)
        ensure_dir(out_dir)

        print("\n" + "#" * 100)
        print(f"Payload case: {case_name}")
        print(f"  payload_mass      = {payload_mass}")
        print(f"  payload_com_local = {payload_com_local}")
        print("#" * 100)

        print("\nRunning ADA-L payload-aware planner...")
        adal = ADALMultiWaypointPlanner(
            ...
            payload_mass=payload_mass, payload_com_local=payload_com_local,
        )
        ...
        print("\nRunning B-spline payload-aware planner...")
        ...
        print_metrics_table(f"ADA-L [{case_name}]",      adal_metrics)
        print_metrics_table(f"B-spline [{case_name}]", bs_metrics)
        print_comparison_table(adal_metrics, bs_metrics, name_a="ADA-L", name_b="B-spline")

        # ---- UR5e trajectory validation ----
        adal_ok, adal_errs, adal_summary = validate_ur5e_trajectory(adal_res["t_q"], adal_res["q"])
        bs_ok,  bs_errs,  bs_summary  = validate_ur5e_trajectory(bs_res["t_q"],  bs_res["q"])
        print_validation_result(f"ADA-L [{case_name}]",      adal_ok, adal_errs, adal_summary)
        print_validation_result(f"B-spline [{case_name}]", bs_ok,  bs_errs,  bs_summary)

        # ---- collision check (capsule-based UR5e self + payload OBB) ----
        _payload_size_xyz = (0.15, 0.15, 0.0175)
        adal_coll_ok, adal_coll_first, adal_coll_n, adal_coll_total = (
            validate_collision(adal_res["q"], payload_size=_payload_size_xyz)
        )
        bs_coll_ok,  bs_coll_first,  bs_coll_n,  bs_coll_total  = (
            validate_collision(bs_res["q"],  payload_size=_payload_size_xyz)
        )
        print_collision_result(
            f"ADA-L [{case_name}]", adal_coll_ok, adal_coll_first, adal_coll_n,
            adal_coll_total, t_traj=adal_res["t_q"],
            q_first=(adal_res["q"][adal_coll_first] if adal_coll_first is not None else None),
            payload_size=_payload_size_xyz,
        )
        print_collision_result(
            f"B-spline [{case_name}]", bs_coll_ok, bs_coll_first, bs_coll_n,
            bs_coll_total, t_traj=bs_res["t_q"],
            q_first=(bs_res["q"][bs_coll_first] if bs_coll_first is not None else None),
            payload_size=_payload_size_xyz,
        )

        print(f"\n=== UR5e Validation Verdict [{case_name}] ===")
        print(f"  ADA-L      : limits {'PASS' if adal_ok else 'FAIL'}, "
              f"collision {'PASS' if adal_coll_ok else 'FAIL'}")
        print(f"  B-spline : limits {'PASS' if bs_ok  else 'FAIL'}, "
              f"collision {'PASS' if bs_coll_ok  else 'FAIL'}")
        ...
        run_config = {
            ...
            "payload_com_local": payload_com_local.copy(),
            "case_name": case_name,
        }
        ...
        # animate_robot_trajectory(
            ...
            # fps=30, title=f"ADA-L trajectory [{case_name}]",
        # )
        # animate_robot_trajectory(
            # robot=robot, q=bs_res["q"], waypoints_xyz=waypoints_xyz,
            # time_grid=bs_res["t_q"],
            # save_path=str(Path(out_dir) / "Bspline_animation.mp4"),
            # fps=30, title=f"B-spline trajectory [{case_name}]",
        # )

        row = {
            "case_name":    case_name,
            "payload_mass": payload_mass,
            "payload_com_x": float(payload_com_local[0]),
            "payload_com_y": float(payload_com_local[1]),
            "payload_com_z": float(payload_com_local[2]),
            "adal_mean_waypoint_error":       float(adal_metrics["mean_waypoint_error"]),
            "bs_mean_waypoint_error":        float(bs_metrics["mean_waypoint_error"]),
            "adal_integrated_squared_torque": float(adal_metrics["integrated_squared_torque"]),
            "bs_integrated_squared_torque":  float(bs_metrics["integrated_squared_torque"]),
            "adal_integrated_joint_abs_work": float(adal_metrics["integrated_joint_abs_work"]),
            "bs_integrated_joint_abs_work":  float(bs_metrics["integrated_joint_abs_work"]),
            "adal_final_velocity_norm":       float(adal_metrics["final_velocity_norm"]),
            "bs_final_velocity_norm":        float(bs_metrics["final_velocity_norm"]),
            "adal_final_acceleration_norm":   float(adal_metrics["final_acceleration_norm"]),
            "bs_final_acceleration_norm":    float(bs_metrics["final_acceleration_norm"]),
            "adal_solve_time": float(adal_metrics["solve_time"]),
            "bs_solve_time":  float(bs_metrics["solve_time"]),
            "adal_ur5e_pass":  bool(adal_ok),
            "bs_ur5e_pass":   bool(bs_ok),
            "adal_collision_pass":     bool(adal_coll_ok),
            "bs_collision_pass":      bool(bs_coll_ok),
            "adal_first_collision_idx": (-1 if adal_coll_first is None else int(adal_coll_first)),
            "bs_first_collision_idx":  (-1 if bs_coll_first  is None else int(bs_coll_first)),
            "adal_collision_count":    int(adal_coll_n),
            "bs_collision_count":     int(bs_coll_n),
        }
        row["work_ratio_adal_over_bs"] = (
            row["adal_integrated_joint_abs_work"] / row["bs_integrated_joint_abs_work"]
            if abs(row["bs_integrated_joint_abs_work"]) > 1e-15 else float("nan")
        )
        row["torque_ratio_adal_over_bs"] = (
            row["adal_integrated_squared_torque"] / row["bs_integrated_squared_torque"]
            if abs(row["bs_integrated_squared_torque"]) > 1e-15 else float("nan")
        )
        all_case_rows.append(row)

    summary_csv = os.path.join(root_out_dir, "payload_case_summary.csv")
    fieldnames = list(all_case_rows[0].keys()) if all_case_rows else []
    ...
    plot_payload_case_summary(
        all_case_rows,
        save_path=os.path.join(root_out_dir, "payload_energy_summary.png"),
        show=False,
    )

    print("\n" + "=" * 100)
    print(f"All payload benchmark outputs saved under: {root_out_dir}")
    print(f"Payload summary CSV: {summary_csv}")
    print("=" * 100)
